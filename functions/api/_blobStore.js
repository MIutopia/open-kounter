...
export function createEmptySystemState() {
  return { token: null, allowedDomains: [], initializedAt: 0, updatedAt: 0, version: '2.0' }
}
...
export function createEmptyCountersDocument() { return { items: {}, updatedAt: 0, version: '2.1' } }
...
export async function replaceAllCounterRecords(store, counters) {
  ...
  const document = await updateCountersDocument(store, () => ({ items, updatedAt: Date.now(), version: '2.1' }))
...
export async function listCounterRecords(store) {
  const document = await loadCountersDocument(store)
  return Object.values(document.items).sort((left, right) => (right.updated_at || 0) - (left.updated_at || 0))
}
...
export function normalizeCounterRecord(target, data) {
  if (!data) return null
  if (typeof data === 'number') return { target, time: Number(data) || 0, created_at: 0, updated_at: 0 }
  if (typeof data === 'string') {
    const count = Number.parseInt(data, 10)
    return { target, time: Number.isNaN(count) ? 0 : count, created_at: 0, updated_at: 0 }
  }
  return { target, time: Number.parseInt(data.time, 10) || 0, created_at: Number(data.created_at) || 0, updated_at: Number(data.updated_at) || 0 }
}
