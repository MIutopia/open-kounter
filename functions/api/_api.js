...
export async function requireAuth(request, store, env) {
  ...
  return { token, state, isAdminToken }
}

export async function validateTokenValue(token, store, env) {
  const state = await loadSystemState(store)
  if (env.ADMIN_TOKEN && token === env.ADMIN_TOKEN) { return { state, isAdminToken: true } }
  if (state.token && token === state.token) { return { state, isAdminToken: false } }
...
export async function loadManagementToken(store, tokenId) {
  ...
  if (value.expiresAt && value.expiresAt <= Date.now()) { await deleteJson(store, key); return null }
  return value
}
